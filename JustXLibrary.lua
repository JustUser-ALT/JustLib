...
local function makeSection(parentFrame,title,parentSg,layoutOrder,variant)
    ...
    local titleLbl=Instance.new("TextLabel"); titleLbl.Size=UDim2.new(1,-58,1,0); titleLbl.Position=UDim2.new(0,14,0,0); titleLbl.BackgroundTransparency=1; titleLbl.Text=title; titleLbl.Font=Enum.Font.GothamBold; titleLbl.TextSize=(variant=="Grid" and 12 or 13); titleLbl.TextColor3=C.txt; titleLbl.TextXAlignment=Enum.TextXAlignment.Left; titleLbl.TextTruncate=Enum.TextTruncate.AtEnd; titleLbl.ZIndex=15; titleLbl.Parent=hdr
...
function JL:Window(opts)
    ...
    function Win:Tab(topts)
        ...
        elseif typ=="Settings" then
            -- Settings gets its own visual language instead of looking like a
            -- normal list of anonymous bars.
            ...
            local function settingsCard(title,desc,order)
                local card=Instance.new("Frame")
                card.Size=UDim2.new(1,0,0,desc and 62 or 46)
                ...
                accent.Position=UDim2.new(0,0,0,9)
                accent.BackgroundColor3=ACCENTS[1]
                accent.BorderSizePixel=0
                accent.ZIndex=14
                accent.Parent=card
                corner(2,accent)

                newTxt({Parent=card,Text=title,Font=Enum.Font.GothamBold,Size=13,
                    Sz=UDim2.new(1,-28,0,22),Pos=UDim2.new(0,16,0,8),Z=15})

                if desc then
                    newTxt({Parent=card,Text=desc,Size=10,Color=C.dim,Wrap=true,
                        Sz=UDim2.new(1,-32,0,25),Pos=UDim2.new(0,16,0,31),Z=15})
                end
            ...
            order+=1
            local intro=settingsCard(
                "JustXLibrary",
                "Tune the look, layout and lifecycle of your hub.",
                order
            )
            intro.Size=UDim2.new(1,0,0,82)
            local introIcon=mkIcon(intro,"⚙",26,15)
            introIcon.Position=UDim2.new(1,-50,0.5,-13)
            introIcon.Size=UDim2.new(0,28,0,28)

            local statusPill=Instance.new("TextLabel")
            statusPill.Size=UDim2.new(0,82,0,22)
            statusPill.Position=UDim2.new(0,16,1,-30)
            statusPill.BackgroundColor3=C.btnBg
            statusPill.BorderSizePixel=0
            statusPill.Text="●  ACTIVE"
            statusPill.Font=Enum.Font.GothamBold
            statusPill.TextSize=9
            statusPill.TextColor3=ACCENTS[1]
            statusPill.ZIndex=15
            statusPill.Parent=intro
            corner(7,statusPill)

            order+=1
            local visual=Instance.new("Frame")
            visual.Size=UDim2.new(1,0,0,114)
            visual.BackgroundColor3=C.panel
            visual.BorderSizePixel=0
            visual.LayoutOrder=order
            visual.ZIndex=13
            visual.Parent=root
            corner(10,visual)
            stroke(C.border,1,visual)

            newTxt({Parent=visual,Text="INTERFACE",Font=Enum.Font.GothamBold,Size=10,Color=C.dim,
                Sz=UDim2.new(1,-24,0,18),Pos=UDim2.new(0,12,0,9),Z=15})

            local fpsRow=Instance.new("Frame")
            fpsRow.Size=UDim2.new(1,-24,0,30)
            fpsRow.Position=UDim2.new(0,12,0,30)
            fpsRow.BackgroundColor3=C.btnBg
            fpsRow.BorderSizePixel=0
            fpsRow.ZIndex=14
            fpsRow.Parent=visual
            corner(7,fpsRow)

            newTxt({Parent=fpsRow,Text="Show FPS",Font=Enum.Font.GothamBold,Size=11,
                Sz=UDim2.new(1,-55,1,0),Pos=UDim2.new(0,10,0,0),Z=15})

            local fpsToggle=Instance.new("TextButton")
            fpsToggle.Size=UDim2.new(0,34,0,18)
            fpsToggle.Position=UDim2.new(1,-44,0.5,-9)
            fpsToggle.BackgroundColor3=cfgGet("_showfps",true) and ACCENTS[1] or C.togOff
            fpsToggle.Text=""
            fpsToggle.ZIndex=15
            fpsToggle.Parent=fpsRow
            corner(9,fpsToggle)

            local fpsKnob=Instance.new("Frame")
            fpsKnob.Size=UDim2.new(0,14,0,14)
            fpsKnob.Position=cfgGet("_showfps",true) and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)
            fpsKnob.BackgroundColor3=Color3.new(1,1,1)
            fpsKnob.BorderSizePixel=0
            fpsKnob.ZIndex=16
            fpsKnob.Parent=fpsToggle
            corner(7,fpsKnob)

            local fpsState=cfgGet("_showfps",true)
            fpsToggle.MouseButton1Click:Connect(function()
                fpsState=not fpsState
                cfgSet("_showfps",fpsState)
                fpsL.Visible=fpsState
                tw(fpsToggle,{BackgroundColor3=fpsState and ACCENTS[1] or C.togOff},.14)
                tw(fpsKnob,{Position=fpsState and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)},.14)
            end)

            local blurRow=Instance.new("Frame")
            blurRow.Size=UDim2.new(1,-24,0,34)
            blurRow.Position=UDim2.new(0,12,0,70)
            blurRow.BackgroundTransparency=1
            blurRow.ZIndex=14
            blurRow.Parent=visual

            newTxt({Parent=blurRow,Text="Interface Blur",Font=Enum.Font.GothamBold,Size=11,
                Sz=UDim2.new(0,95,1,0),Pos=UDim2.new(0,0,0,0),Z=15})

            local blurValue=cfgGet("_blur",0) or 0
            local blurValueLabel=newTxt({Parent=blurRow,Text=tostring(math.round(blurValue)).."%",Font=Enum.Font.GothamBold,Size=10,Color=C.dim,
                XAlign=Enum.TextXAlignment.Right,Sz=UDim2.new(0,40,1,0),Pos=UDim2.new(1,-40,0,0),Z=15})

            local blurTrack=Instance.new("TextButton")
            blurTrack.Size=UDim2.new(1,-145,0,6)
            blurTrack.Position=UDim2.new(0,100,0.5,-3)
            blurTrack.BackgroundColor3=C.slTrack
            blurTrack.Text=""
            blurTrack.ZIndex=15
            blurTrack.Parent=blurRow
            corner(3,blurTrack)

            local blurFill=Instance.new("Frame")
            blurFill.Size=UDim2.new(blurValue/100,0,1,0)
            blurFill.BackgroundColor3=ACCENTS[1]
            blurFill.BorderSizePixel=0
            blurFill.ZIndex=16
            blurFill.Parent=blurTrack
            corner(3,blurFill)

            local blurDragging=false
            local function setBlur(v)
                blurValue=math.clamp(math.round(v),0,100)
                cfgSet("_blur",blurValue)
                blurValueLabel.Text=tostring(blurValue).."%"
                blurFill.Size=UDim2.new(blurValue/100,0,1,0)
                ...
                if blurValue>0 then
                    ...
                    b.Enabled=JL._winOpen
                elseif b then
                    b.Enabled=false
                end
            end

            blurTrack.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
                    blurDragging=true
                    local p=(i.Position.X-blurTrack.AbsolutePosition.X)/math.max(1,blurTrack.AbsoluteSize.X)
                    setBlur(p*100)
                end
            end)
            UIS.InputChanged:Connect(function(i)
                if blurDragging and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
                    local p=(i.Position.X-blurTrack.AbsolutePosition.X)/math.max(1,blurTrack.AbsoluteSize.X)
                    setBlur(p*100)
                end
            end)
            ...
            badge.Size=UDim2.new(1,0,0,132)
            ...
            newTxt({Parent=badge,Text="FLOATING BADGE",Font=Enum.Font.GothamBold,Size=10,Color=C.dim,
                Sz=UDim2.new(1,-24,0,18),Pos=UDim2.new(0,12,0,9),Z=15})

            local positions={{"Top","top"},{"Center","center"},{"Bottom","bottom"}}
            for i,item in ipairs(positions) do
                local b=Instance.new("TextButton")
                b.Size=UDim2.new(1,-24,0,25)
                b.Position=UDim2.new(0,12,0,35+(i-1)*29)
            ...
            danger.Size=UDim2.new(1,0,0,98)
            danger.BackgroundColor3=C.panel
            danger.BorderSizePixel=0
            danger.LayoutOrder=order
            danger.ZIndex=13
            danger.Parent=root
            corner(10,danger)
            stroke(C.border,1,danger)

            newTxt({Parent=danger,Text="SESSION",Font=Enum.Font.GothamBold,Size=10,Color=C.dim,
                Sz=UDim2.new(1,-24,0,18),Pos=UDim2.new(0,12,0,9),Z=15})

            local closeBtn=Instance.new("TextButton")
            closeBtn.Size=UDim2.new(1,-24,0,27)
            closeBtn.Position=UDim2.new(0,12,0,35)
            closeBtn.BackgroundColor3=Color3.fromRGB(100,45,48)
            closeBtn.Text="Unload JustXLibrary"
            closeBtn.Font=Enum.Font.GothamBold
            closeBtn.TextSize=10
            closeBtn.TextColor3=C.txt
            closeBtn.ZIndex=15
            closeBtn.Parent=danger
            corner(7,closeBtn)
            ...
            layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                root.Size=UDim2.new(1,0,0,layout.AbsoluteContentSize.Y+12)
            ...
            function Tab:Section()
                error("Settings tabs do not use Section(); use Window:Settings() or the built-in Settings tab.",2)
            end
