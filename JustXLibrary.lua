...
function JL:Window(opts)
    ...
    function Win:Tab(topts)
        ...
        local scroll=Instance.new("ScrollingFrame"); scroll.Size=UDim2.new(1,0,1,0); scroll.BackgroundTransparency=1; scroll.BorderSizePixel=0; scroll.ScrollBarThickness=3; scroll.ScrollBarImageColor3=ACCENTS[1]; scroll.CanvasSize=UDim2.new(0,0,0,0); scroll.ZIndex=12; scroll.Parent=tabFrame; pad(0,2,26,8,scroll)
        local Tab={}
        if typ=="Grid" then
            -- Grid 2.0:
        ...
        elseif typ=="Settings" then
            -- Settings 2.0: dashboard layout with three readable columns.
            local root=Instance.new("Frame")
            root.Size=UDim2.new(0,0,0,0); root.BackgroundTransparency=1; root.Parent=scroll
            local grid=Instance.new("UIGridLayout")
            grid.CellPadding=UDim2.new(0,10,0,10)
            grid.CellSize=UDim2.new(0.333,-7,0,0)
            ...
            local function card(title,kicker,order,height,accent)
                local f=Instance.new("Frame"); f.BackgroundColor3=C.panel; f.BorderSizePixel=0; f.LayoutOrder=order; f.ZIndex=13; f.Parent=root; corner(11,f); stroke(C.border,1,f)
                local a=Instance.new("Frame"); a.Size=UDim2.new(0,3,0,34); a.Position=UDim2.new(0,0,0,12); a.BackgroundColor3=accent or ACCENTS[1]; a.BorderSizePixel=0; a.ZIndex=15; a.Parent=f; corner(2,a)
                newTxt({Parent=f,Text=kicker or "SETTINGS",Font=Enum.Font.GothamBold,Size=9,Color=C.dim,Sz=UDim2.new(1,-28,0,14),Pos=UDim2.new(0,14,0,10),Z=15})
                local host=textHost(f,14,27,math.max(60,root.AbsoluteSize.X-28),20,15); host.Size=UDim2.new(1,-28,0,20)
                longText(f,title,{Host=host,Size=13,Z=15,TextMode="Scroll"})
                f:SetAttribute("DesiredHeight",height)
                return f
            end
            local cards={}
            local function addCard(title,kicker,order,height,accent)
                local f=card(title,kicker,order,height,accent); table.insert(cards,f); return f
            end
            local function makeSwitch(parent,y,name,flag,default)
                local row=Instance.new("TextButton"); row.Size=UDim2.new(1,-20,0,32); row.Position=UDim2.new(0,10,0,y); row.BackgroundColor3=C.btnBg; row.Text=""; row.ZIndex=15; row.Parent=parent; corner(7,row)
                local host=textHost(row,9,0,math.max(40,row.AbsoluteSize.X-52),32,16); host.Size=UDim2.new(1,-52,1,0); longText(row,name,{Host=host,Size=11,Z=16,TextMode="Scroll"})
                local state=cfgGet(flag,default); local sw=Instance.new("Frame"); sw.Size=UDim2.new(0,34,0,18); sw.Position=UDim2.new(1,-43,0.5,-9); sw.BackgroundColor3=state and ACCENTS[1] or C.togOff; sw.ZIndex=16; sw.Parent=row; corner(9,sw)
                local knob=Instance.new("Frame"); knob.Size=UDim2.new(0,14,0,14); knob.Position=state and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7); knob.BackgroundColor3=Color3.new(1,1,1); knob.BorderSizePixel=0; knob.ZIndex=17; knob.Parent=sw; corner(7,knob)
                row.MouseButton1Click:Connect(function() state=not state; cfgSet(flag,state); if flag=="_showfps" then fpsL.Visible=state end; if flag=="_blurEnabled" then local LT=game:GetService("Lighting"); local be=LT:FindFirstChildOfClass("BlurEffect"); if state and (cfgGet("_blur",0) or 0)>0 then if not be then be=Instance.new("BlurEffect"); be.Parent=LT end; be.Size=math.round((cfgGet("_blur",0) or 0)*56/100); be.Enabled=JL._winOpen else if be then be.Enabled=false end end end; tw(sw,{BackgroundColor3=state and ACCENTS[1] or C.togOff},.14); tw(knob,{Position=state and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)},.14) end)
            end
            local function makeAction(parent,y,name,callback,destructive)
                local b=Instance.new("TextButton"); b.Size=UDim2.new(1,-20,0,34); b.Position=UDim2.new(0,10,0,y); b.BackgroundColor3=destructive and Color3.fromRGB(100,45,48) or C.btnBg; b.Text=""; b.ZIndex=15; b.Parent=parent; corner(7,b); stroke(C.border,1,b)
                longText(b,name,{Host=textHost(b,10,0,math.max(40,b.AbsoluteSize.X-20),34,16),Size=11,XAlign=Enum.TextXAlignment.Center,Z=16,TextMode="Scroll"})
                b.MouseEnter:Connect(function() tw(b,{BackgroundColor3=destructive and Color3.fromRGB(125,55,58) or C.btnHov},.1) end); b.MouseLeave:Connect(function() tw(b,{BackgroundColor3=destructive and Color3.fromRGB(100,45,48) or C.btnBg},.1) end); b.MouseButton1Click:Connect(callback)
            end

            local interface=addCard("Interface","APPEARANCE",1,120,ACCENTS[1])
            makeSwitch(interface,42,"Show FPS","_showfps",true)
            makeSwitch(interface,78,"Interface blur","_blurEnabled",(cfgGet("_blur",0) or 0)>0)

            local badge=addCard("Floating Badge","LAYOUT",2,168,ACCENTS[2])
            newTxt({Parent=badge,Text="Position",Font=Enum.Font.GothamBold,Size=10,Color=C.dim,Sz=UDim2.new(1,-20,0,16),Pos=UDim2.new(0,10,0,43),Z=15})
            local positions={{"Top","top"},{"Center","center"},{"Bottom","bottom"}}
            for i,item in ipairs(positions) do
                local b=Instance.new("TextButton"); b.Size=UDim2.new(1,-20,0,27); b.Position=UDim2.new(0,10,0,62+(i-1)*30); b.BackgroundColor3=(cfgGet("_badgepos","top")==item[2]) and Color3.fromRGB(18,36,62) or C.btnBg; b.Text=item[1]; b.Font=Enum.Font.GothamBold; b.TextSize=10; b.TextColor3=C.txt; b.ZIndex=15; b.Parent=badge; corner(6,b)
                b.MouseButton1Click:Connect(function() cfgSet("_badgepos",item[2]); tw(badge2,{Position=BPOS[item[2]]},.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out); for _,ch in ipairs(badge:GetChildren()) do if ch:IsA("TextButton") then ch.BackgroundColor3=C.btnBg end end; b.BackgroundColor3=Color3.fromRGB(18,36,62) end)
            end

            local blur=addCard("Blur Amount","EFFECTS",3,106,ACCENTS[3])
            local blurValue=cfgGet("_blur",0) or 0
            local valLbl=newTxt({Parent=blur,Text=tostring(math.round(blurValue)).."%",Font=Enum.Font.GothamBold,Size=12,Color=C.txt,XAlign=Enum.TextXAlignment.Right,Sz=UDim2.new(0,45,0,18),Pos=UDim2.new(1,-55,0,42),Z=15})
            local track=Instance.new("TextButton"); track.Size=UDim2.new(1,-30,0,8); track.Position=UDim2.new(0,15,0,73); track.BackgroundColor3=C.slTrack; track.Text=""; track.ZIndex=15; track.Parent=blur; corner(4,track)
            local fill=Instance.new("Frame"); fill.Size=UDim2.new(blurValue/100,0,1,0); fill.BackgroundColor3=ACCENTS[3]; fill.BorderSizePixel=0; fill.ZIndex=16; fill.Parent=track; corner(4,fill)
            local drag=false
            local function setBlur(v)
                blurValue=math.clamp(math.round(v),0,100); cfgSet("_blur",blurValue); valLbl.Text=tostring(blurValue).."%"; fill.Size=UDim2.new(blurValue/100,0,1,0)
                local LT=game:GetService("Lighting"); local b=LT:FindFirstChildOfClass("BlurEffect")
                if blurValue>0 then if not b then b=Instance.new("BlurEffect"); b.Parent=LT end; b.Size=math.round(blurValue*56/100); b.Enabled=JL._winOpen else if b then b.Enabled=false end end
            end
            track.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=true; setBlur((i.Position.X-track.AbsolutePosition.X)/math.max(1,track.AbsoluteSize.X)*100) end end)
            UIS.InputChanged:Connect(function(i) if drag and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then setBlur((i.Position.X-track.AbsolutePosition.X)/math.max(1,track.AbsoluteSize.X)*100) end end)
            UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=false end end)

            local info=addCard("Library","ABOUT",4,132,ACCENTS[4])
            newTxt({Parent=info,Text="JustXLibrary",Font=Enum.Font.GothamBold,Size=12,Color=C.txt,Sz=UDim2.new(1,-20,0,18),Pos=UDim2.new(0,10,0,43),Z=15})
            newTxt({Parent=info,Text="Modern UI / Grid 2.0",Size=10,Color=C.dim,Sz=UDim2.new(1,-20,0,18),Pos=UDim2.new(0,10,0,62),Z=15})
            makeAction(info,90,"Unload JustXLibrary",function() local sure=confirmDialog("Unload Library","This will completely remove the current hub."); if sure then destroyHub() end end,true)

            local empty=addCard("Quick Actions","TOOLS",5,132,ACCENTS[5])
            makeAction(empty,43,"Reset Badge Position",function() cfgSet("_badgepos","top"); tw(badge2,{Position=BPOS.top},.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out) end,false)
            makeAction(empty,84,"Reset Interface Blur",function() setBlur(0) end,false)

            local function rebuildSettings()
                local width=math.max(300,scroll.AbsoluteSize.X-4)
                local cols=(width>=620 and 3) or 2
                local gap=10
                local cw=(width-gap*(cols-1))/cols
                grid.CellSize=UDim2.new(0,cw,0,0)
                local rowHeights={}
                for _,f in ipairs(cards) do f.Size=UDim2.new(0,cw,0,f:GetAttribute("DesiredHeight") or 100) end
                for i,f in ipairs(cards) do
                    local r=math.floor((i-1)/cols)+1; rowHeights[r]=math.max(rowHeights[r] or 0,f.Size.Y.Offset)
                end
                local totalH=0; for _,h in ipairs(rowHeights) do totalH=totalH+h+gap end
                root.Size=UDim2.new(0,width,0,math.max(0,totalH-gap+12)); scroll.CanvasSize=UDim2.new(0,width,0,math.max(0,totalH-gap+20))
            end
            local settingsResize=scroll:GetPropertyChangedSignal("AbsoluteSize"):Connect(rebuildSettings); table.insert(gridConnections,settingsResize)
            task.defer(rebuildSettings)
            function Tab:Section() error("Settings tabs use the built-in dashboard.",2) end
        else
