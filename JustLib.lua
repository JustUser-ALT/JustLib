...
-- ============================== custom icon packs ==============================
-- Roblox can only render rbxassetid:// images, not arbitrary URLs (github/lucide/figma links, etc).
-- What WindUI-style libraries actually do is look icon *names* up in a pre-uploaded name -> rbxassetid
-- table. JL:LoadIconPack fetches exactly that kind of table (a raw .json, or a Lua module that
-- `return`s a table) from a URL, e.g. a pack built from https://github.com/Footagesus/Icons.
-- Once loaded, every Icon = "..." field anywhere in the library (Window/Tab/Section/Toggle/Button/...)
-- is looked up in it automatically; anything not found falls back to being treated as a literal
-- rbxassetid/asset id/text glyph like before, so this is fully backwards compatible.
function JL:LoadIconPack(url)
    ...
    if url:match("%.json$") then
        ...
        if ok2 and fn then
            local ok3,d = pcall(fn)
            if ok3 then data = d end
        end
    end
...
-- merge/override individual names without replacing the whole pack, e.g. JL:SetIcons({heart = 123456})
function JL:SetIcons(tbl)
    JL._iconPack = JL._iconPack or {}
    for k,v in pairs(tbl or {}) do JL._iconPack[k] = v end
end
local function resolveIcon(s)
    if JL._iconPack and JL._iconPack[s] ~= nil then return tostring(JL._iconPack[s]) end
    return s
end
...
-- ============================== notifications ==============================
-- Configurable via Settings > Notifications: which types show (_notify_show_regular/_warning/_error),
-- default duration (_notify_duration), screen corner (_notify_side), and de-dupe stacking (_notify_stack).
do
    ...
    function JL:Notify(cfg2)
        ...
        -- Notify Stack: fold an identical (title+desc+type) notification into an "x2, x3..." counter
        ...
        newTxt({Parent=f,Text=title,Font=GB,Size=14,Color=Color3.new(1,1,1),Sz=UDim2.new(0,142,0,20),Pos=UDim2.new(0,22,0,2),Z=5})
        local countLbl = newTxt({Parent=f,Text="",Font=GB,Size=12,Color=col,XAlign=Enum.TextXAlignment.Right,Sz=UDim2.new(0,26,0,20),Pos=UDim2.new(1,-28,0,2),Z=5})
...
local function attachWidgets(content,INNER,ac,uiScale,parentSg,attachTip)
    ...
    local Sec = {}
    local ICONW = 22
    -- draws an optional leading icon inside `holder` (a button/frame) and returns the width it consumes
...
function JL:Window(opts)
    ...
    -- side is picked in Settings > Interface > Tooltip Position (_tooltip_side: Top/Bottom/Left/Right)
