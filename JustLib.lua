--[[
    ...
    - New: Keybind, Segmented, slider Step/Suffix/typed values, tab tooltips, UI Scale, two-pane Settings
]]
local JL = {}; JL.Flags = {}; JL._winOpen = false; JL._listening = false; JL.Version = "2.0"
...
-- ============================== section ==============================
local function makeSection(parentFrame,title,parentSg,layoutOrder,colW,so)
    so = so or {}
    local ac = nxAc()
    ...
    function Sec:Toggle(o)
        ...
        return {Set = function(_,v) state = v; sw.Set(v); cfgSet(o.Flag, v) end, Get = function() return state end}
    end
    ...
    -- ---------- Dropdown (wrapped options, shows selected values) ----------
    function Sec:Dropdown(o)
        ...
        local TOTAL = math.min(total, 160); local dropOpen = false
    ...
    function Sec:ColorPicker(o)
        ...
        return {Get = function() return cur end, Set = function(_,v) cur = v; sw.BackgroundColor3 = v end}
    end
    ...
    function Sec:Keybind(o)
        ...
        return {Get = function() return chip.Get() end, Set = function(_,k) chip.Set(k); cfgSet(o.Flag, k and k.Name or "None") end}
    end
    ...
    function Sec:Segmented(o)
        ...
        if o.Callback and had then pcall(o.Callback, cur) end
        return {Get = seg.Get, Set = function(_,v) seg.Set(v, true); cfgSet(o.Flag, v) end}
    end
...
function JL:Window(opts)
    ...
    local badge2 = Instance.new("Frame"); badge2.Size = UDim2.new(0,158,0,30); badge2.Position = BPOS[tostring(cfgGet("_badgepos","Top")):lower()] or BPOS.top
    ...
    newTxt({Parent=badge2,Text=opts.Title or "JustLib",Font=GB,Size=12,Sz=UDim2.new(0,78,1,0),Pos=UDim2.new(0,offX,0,0),Z=61})
    ...
    local fpsL = newTxt({Parent=badge2,Text="60 FPS",Font=GB,Size=12,Color=C.fpsGrn,XAlign=Enum.TextXAlignment.Right,Sz=UDim2.new(0,46,1,0),Pos=UDim2.new(0,104,0,0),Z=61})
    ...
    function Win:Tab(topts)
        ...
        if typ == "Grid" then
            ...
            local function relayout()
                local mH = 0
                for i = 1,ncols do
                    local hh = cols[i].L.AbsoluteContentSize.Y
            ...
            function Tab:Section(sopts)
                ...
                return makeSection(getCol(ci).F, sopts.Title or "Section", sg, secCount[ci], colW, sopts)
            end
        ...
        elseif typ == "Custom" then
            ...
            function Tab:Section(sopts)
                ...
                return makeSection(listCon, sopts.Title or "Section", sg, _sOrd, listW, sopts)
            end
        end
        return Tab
    end
    ...
    function Win:Settings()
        ...
        -- ---- Config ----
        local pC = newPage("Config", "Saved settings")
        addValue(pC, "Config file", "Where your flags are stored", _cfgFile or "not set (nothing is saved)")
