--[[
    JustLib 2.1
    - Grid: N numbered columns (Column=1,2,3,4...), horizontal paging when columns > visible,
      multiple :Section() calls with the same Column stack vertically underneath each other
    - Every row auto-grows in height, so long text is wrapped instead of cut off
    - UI Scale is now double-scale-safe (sidebar / grid columns / settings pages / dropdowns
      no longer overflow or become unreachable at scale != 100%)
    - New this version: MultiSection (one card, switchable pages, e.g. "Camera | Effects"),
      Text, Screen (image / decal / asset id / player headshot), Dropdown Search = true,
      NoSave option on any widget, and a full named-Presets system in Settings
      (Save As New / Load / Rewrite / Delete / Auto Load) separate from the auto-saved file
]]
local JL = {}; JL.Flags = {}; JL._winOpen = false; JL._listening = false; JL.Version = "2.1"
...
do
    ...
    function JL:Notify(cfg2)
        ...
        newTxt({Parent=f,Text=title,Font=GB,Size=14,Color=Color3.new(1,1,1),Sz=UDim2.new(0,170,0,20),Pos=UDim2.new(0,22,0,2),Z=5})
        ...
        local cb=Instance.new("TextButton"); cb.BorderSizePixel=0; cb.BackgroundTransparency=1; cb.TextSize=14; cb.Font=GB; cb.TextColor3=Color3.new(1,1,1); cb.Size=UDim2.new(0,20,0,20); cb.Position=UDim2.new(1,-22,0,2); cb.Text="x"; cb.Parent=f
...
local function confirmDialog(promptTitle,promptDesc)
    ...
    local bd = Instance.new("Frame"); bd.Size = UDim2.new(1,0,1,0); bd.BackgroundColor3 = Color3.new(0,0,0); bd.BackgroundTransparency = 0.45; bd.BorderSizePixel = 0; bd.Active = true; bd.Parent = sg3
    local card = Instance.new("Frame"); card.Size = UDim2.new(0,280,0,150); card.Position = UDim2.new(0.5,-140,0.5,-75); card.BackgroundColor3 = C.panel; card.BorderSizePixel = 0; card.Parent = bd; corner(12,card); stroke(C.border,1,card)
    newTxt({Parent=card,Text=promptTitle,Font=GB,Size=15,XAlign=Enum.TextXAlignment.Center,Sz=UDim2.new(1,-20,0,20),Pos=UDim2.new(0,10,0,14)})
    newTxt({Parent=card,Text=promptDesc,Size=11,Color=C.dim,Wrap=true,XAlign=Enum.TextXAlignment.Center,Sz=UDim2.new(1,-28,0,52),Pos=UDim2.new(0,14,0,40)})
    local function mkB(txt,col,pos)
        ...
        b.TextColor3 = Color3.new(1,1,1); b.AutoButtonColor = true; b.Parent = card; corner(8,b); return b
    end
    local yesBtn = mkB("Yes", Color3.fromRGB(72,198,138), UDim2.new(0,14,1,-46))
    local noBtn  = mkB("No",  C.danger,                   UDim2.new(0.5,8,1,-46))
    local result = nil
    yesBtn.MouseButton1Click:Connect(function() if result == nil then result = true end end)
    noBtn.MouseButton1Click:Connect(function() if result == nil then result = false end end)
...
local function attachWidgets(content,INNER,ac,uiScale,parentSg)
    ...
    function Sec:Toggle(o)
        ...
        local name = o.Name or "Toggle"; local labW = INNER - 10 - 48
        ...
        corner(7,bg); stroke(C.border,1,bg); hover(bg, C.btnBg, C.btnHov)
        newTxt({Parent=bg,Text=name,Font=GB,Size=12,Wrap=true,Sz=UDim2.new(0,labW,1,0),Pos=UDim2.new(0,10,0,0)})
    ...
    function Sec:Slider(o)
        ...
        local topH = fitH(name, 12, GB, INNER - 4 - 66, 18, 2)
        local row = newRow(topH + 24)
        newTxt({Parent=row,Text=name,Font=GB,Size=12,Wrap=true,Sz=UDim2.new(1,-66,0,topH),Pos=UDim2.new(0,4,0,0)})
    ...
    function Sec:Button(o)
        o = o or {}
        local row = newRow(fitH(o.Name or "Button", 12, GB, INNER - 20, 32, 14))
        ...
        btn.Text = o.Name or "Button"; btn.Font = GB; btn.TextSize = 12; btn.TextColor3 = C.txt; btn.TextWrapped = true; btn.Parent = row
        corner(7,btn); stroke(C.border,1,btn); hover(btn, C.btnBg, C.btnHov)
        btn.MouseButton1Click:Connect(function()
    ...
    function Sec:Input(o)
        ...
        local box = Instance.new("TextBox"); box.Size = UDim2.new(1,-16,1,0); box.Position = UDim2.new(0,8,0,0); box.BackgroundTransparency = 1
    ...
    function Sec:Dropdown(o)
        ...
        local baseName = o.Name or "Dropdown"; local labW = INNER - 10 - 30
        ...
        corner(7,bg); stroke(C.border,1,bg); hover(bg, C.btnBg, C.btnHov)
        local lbl = newTxt({Parent=bg,Text=labelText(),Font=GB,Size=12,Wrap=true,Sz=UDim2.new(0,labW,1,0),Pos=UDim2.new(0,10,0,0)})
        ...
        local function mark(ref,on) ref.mark.Visible = on; ref.opt.TextColor3 = on and ac or C.txt end
        for i,optName in ipairs(options) do
            ...
            opt.Text = tostring(optName); opt.Font = GB; opt.TextSize = 11; opt.TextColor3 = C.txt; opt.TextWrapped = true; opt.LayoutOrder = i; opt.Parent = container; corner(5,opt)
            ...
            optRefs[optName] = {opt = opt, mark = mk}; mark(optRefs[optName], selected[optName] or false)
    ...
    function Sec:ColorPicker(o)
        ...
        local name = o.Name or "Color"; local labW = INNER - 10 - 52
        local row = newRow(fitH(name, 12, GB, labW, 32, 14))
        local bg = Instance.new("TextButton"); bg.Size = UDim2.new(1,0,1,0); bg.BackgroundColor3 = C.btnBg; bg.Text = ""; bg.AutoButtonColor = false; bg.BorderSizePixel = 0; bg.Parent = row
        corner(7,bg); stroke(C.border,1,bg); hover(bg, C.btnBg, C.btnHov)
        newTxt({Parent=bg,Text=name,Font=GB,Size=12,Wrap=true,Sz=UDim2.new(0,labW,1,0),Pos=UDim2.new(0,10,0,0)})
    ...
    function Sec:Keybind(o)
        ...
        local name = o.Name or "Keybind"; local labW = INNER - 10 - 96
        local row = newRow(fitH(name, 12, GB, labW, 32, 14))
    ...
    function Sec:Text(o)
        ...
        local h0 = fitH(o.Text or "", sz, fnt, INNER - 8, 16, 6)
        local row = newRow(h0)
        local l = newTxt({Parent=row,Text=o.Text or "",Size=sz,Color=o.Color or C.txt,Wrap=true,XAlign=o.Align or Enum.TextXAlignment.Left,Sz=UDim2.new(1,-8,1,0),Pos=UDim2.new(0,4,0,0)})
        l.Font = fnt
        if o.RichText then l.RichText = true end
        return {Set = function(_,t) l.Text = t; row.Size = UDim2.new(1,0,0,fitH(t, sz, fnt, INNER - 8, 16, 6)) end}
    end
    ...
    function Sec:Label(o)
        o = o or {}
        local h0 = fitH(o.Text or "", 11, G, INNER - 8, 18, 4)
        local row = newRow(h0)
        local l = newTxt({Parent=row,Text=o.Text or "",Size=11,Color=o.Color or C.dim,Wrap=true,Sz=UDim2.new(1,-8,1,0),Pos=UDim2.new(0,4,0,0)})
        if o.RichText then l.RichText = true end
        return {Set = function(_,t) l.Text = t; row.Size = UDim2.new(1,0,0,fitH(t, 11, G, INNER - 8, 18, 4)) end}
    end
...
-- ============================== section (single card) ==============================
local function makeSection(parentFrame,title,parentSg,layoutOrder,colW,so,uiScale)
    so = so or {}
    local ac = nxAc()
    local INNER = colW - 16                                   -- usable row width
    local HH = math.max(34, fitH(title, 13, GB, colW - 54, 0, 0) + 16)   -- header grows if the title is long
    ...
    newTxt({Parent=hdr,Text=title,Font=GB,Size=13,Wrap=true,Sz=UDim2.new(1,-54,1,0),Pos=UDim2.new(0,22,0,0),Z=3})
    ...
    local Sec = attachWidgets(content, INNER, ac, uiScale, parentSg)
    Sec.Panel = panel
...
local function makeMultiSection(parentFrame,sopts,parentSg,layoutOrder,colW,uiScale)
    sopts = sopts or {}
    local ac = nxAc()
    local INNER = colW - 16
    ...
    local switcher = mkSegmented(hdr, UDim2.new(0,8,0,6), UDim2.new(1,-16,0,28), pages, activePage, ac, function(v)
    ...
    for _,name in ipairs(pages) do
        ...
        pageSecs[name] = attachWidgets(content, INNER, ac, uiScale, parentSg)
    end
...
function JL:Window(opts)
    ...
    -- ---------- config profiles: named snapshots of feature flags (separate from the auto-saved file) ----------
    ...
    -- IMPORTANT: AbsoluteContentSize is already post-UIScale (real screen pixels). Any time we feed it back
    -- into a Size/Offset of something INSIDE the scaled tree, it must be divided by the current scale first,
    -- or it gets applied twice and things shrink/overflow their containers at scales other than 100%.
    ...
    -- tab tooltip
    local tip = Instance.new("Frame"); tip.Size = UDim2.new(0,80,0,24); tip.BackgroundColor3 = Color3.fromRGB(18,18,20); tip.BorderSizePixel = 0; tip.Visible = false; tip.ZIndex = 50; tip.Parent = win
    corner(6,tip); stroke(C.border,1,tip)
    local tipL = newTxt({Parent=tip,Text="",Font=GB,Size=11,XAlign=Enum.TextXAlignment.Center,Z=51})
    local function showTip(btn,text)
        local w = TextService:GetTextSize(text, 11, GB, Vector2.new(400,24)).X + 18
        local rel = (btn.AbsolutePosition - win.AbsolutePosition) / uiScale.Scale
        tip.Size = UDim2.new(0,w,0,24); tip.Position = UDim2.new(0,SBW + 4,0,rel.Y + 3); tipL.Text = text; tip.Visible = true
    end
    ...
    function Win:Tab(topts)
        ...
        if topts.Name then
            btn.MouseEnter:Connect(function() showTip(btn, topts.Name) end)
        ...
        if typ == "Grid" then
            ...
            -- pager (only shown when there are more columns than fit)
            ...
            local function rebuildPager()
                ...
                mkChip("‹", 0, function() goCol(curCol() - 1) end)
                for i = 1,ncols do chips[i] = mkChip(tostring(i), i, function() goCol(i) end) end
                mkChip("›", ncols + 1, function() goCol(curCol() + 1) end)
                paintChips()
            end
            ...
            function Tab:Section(sopts)
                ...
                return makeSection(getCol(ci).F, sopts.Title or "Section", sg, secCount[ci], colW, sopts, uiScale)
            end
            function Tab:MultiSection(sopts)
                sopts = sopts or {}
                local ci = resolveCol(sopts.Column)
                secCount[ci] = (secCount[ci] or 0) + 1
                return makeMultiSection(getCol(ci).F, sopts, sg, secCount[ci], colW, uiScale)
            end
        ...
        elseif typ == "Custom" then
            ...
            function Tab:Section(sopts)
                ...
                return makeSection(listCon, sopts.Title or "Section", sg, _sOrd, listW, sopts, uiScale)
            end
            function Tab:MultiSection(sopts)
                sopts = sopts or {}; _sOrd = _sOrd + 1
                return makeMultiSection(listCon, sopts, sg, _sOrd, listW, uiScale)
            end
        end
        return Tab
    end
    ...
    function Win:Settings()
        ...
        addButton(pP, "Delete Selected", "Permanently deletes the selected preset", "Delete", true, function()
            ...
            if confirmDialog("Delete Preset", "Permanently delete \"" .. nm .. "\"?") then
                deleteProfile(nm)
        ...
        addButton(pC, "Reset interface", "Restores every setting on the Appearance and Interface pages", "Reset", true, function()
            if confirmDialog("Reset Settings", "Restore all interface settings to their defaults?") then
        ...
        addButton(pB, "Close Hub", "Fully unloads the hub", "Unload", true, function()
            if confirmDialog("Close Hub", "Are you sure you want to close the hub? This will fully unload it.") then destroyHub() end
        end)
    ...
    -- runs once the whole synchronous build of this script (Tabs/Sections/widgets) has finished,
    -- so every Flag is registered before we try to apply a preset to it
    task.defer(function()
        if _dead then return end
