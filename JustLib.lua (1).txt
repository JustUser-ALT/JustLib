...
local function makeSection(parentFrame,title,parentSg)
    ...
    local function resizePanel()
        local h=list.AbsoluteContentSize.Y+14; content.Size=UDim2.new(1,-6,0,h); panel.Size=UDim2.new(1,0,0,36+h)
    end
    list:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(resizePanel)
    -- Collapse
    local collapsed=false; local lastH=0
    cb.MouseButton1Click:Connect(function()
        collapsed=not collapsed; cb.Text=collapsed and "↑" or "↓"
        if collapsed then lastH=list.AbsoluteContentSize.Y+14; tw(panel,{Size=UDim2.new(1,0,0,38)},.18); task.delay(.18,function() if collapsed then content.Visible=false end end)
        else content.Visible=true; tw(panel,{Size=UDim2.new(1,0,0,36+lastH)},.18) end
    end)
    local iOrd=0
    local Sec={}
    ...
    function Sec:Dropdown(opts)
        ...
        local dw=Instance.new("Frame"); dw.Size=UDim2.new(1,0,0,HDR); dw.BackgroundColor3=C.btnBg; dw.BorderSizePixel=0; dw.ZIndex=17; dw.ClipsDescendants=true; dw.Parent=dRow; corner(7,dw); stroke(C.border,1,dw)
        ...
        local oFr=Instance.new("Frame"); oFr.Size=UDim2.new(1,0,0,TOTAL); oFr.Position=UDim2.new(0,0,0,HDR); oFr.BackgroundTransparency=1; oFr.ZIndex=18; oFr.Parent=dw
        for i,optName in ipairs(options) do
            local oRow=Instance.new("Frame"); oRow.Size=UDim2.new(1,0,0,OPT); oRow.Position=UDim2.new(0,0,0,(i-1)*OPT); oRow.BackgroundTransparency=1; oRow.ZIndex=19; oRow.Parent=oFr
            local chk=Instance.new("Frame"); chk.Size=UDim2.new(0,13,0,13); chk.Position=UDim2.new(0,8,0.5,-6); chk.BackgroundColor3=selected[optName] and C.togOn or C.chkBg; chk.BorderSizePixel=0; chk.ZIndex=20; chk.Parent=oRow; corner(3,chk)
            local chkM=newTxt({Parent=chk,Text=selected[optName] and "✓" or "",Font=Enum.Font.GothamBold,Size=9,Color=Color3.new(1,1,1),Z=21})
            newTxt({Parent=oRow,Text=optName,Sz=UDim2.new(1,-28,1,0),Pos=UDim2.new(0,26,0,0),Z=20})
            local rB=Instance.new("TextButton"); rB.Size=UDim2.new(1,0,1,0); rB.BackgroundTransparency=1; rB.Text=""; rB.ZIndex=21; rB.Parent=oRow
            rB.MouseButton1Click:Connect(function()
                if multi then
                    ...
                    for _,oR2 in ipairs(oFr:GetChildren()) do if oR2:IsA("Frame") then local ck2=oR2:FindFirstChildOfClass("Frame"); local cm2=oR2:FindFirstChildOfClass("TextLabel"); if ck2 then tw(ck2,{BackgroundColor3=C.chkBg},.1) end; if cm2 then cm2.Text="" end end end
        ...
        hBtn.MouseButton1Click:Connect(function()
            ...
            if expanded then dRow.Size=UDim2.new(1,0,0,HDR+TOTAL); dw.Size=UDim2.new(1,0,0,HDR+TOTAL) else dRow.Size=UDim2.new(1,0,0,HDR); dw.Size=UDim2.new(1,0,0,HDR) end
        end)
...
function JL:Window(opts)
    ...
    local sg=Instance.new("ScreenGui"); sg.Name="JustLib"; sg.ResetOnSpawn=false; sg.IgnoreGuiInset=true; sg.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; sg.Parent=PG
    -- Badge
